  ...
  function MagicHomeControlNode(config) {
    ...
    class LampState extends events.EventEmitter {
      ...
      static normalizeRange(min, max, value, previousValue) {
        ...
        if (
          !(Number.isInteger(min) && Number.isInteger(max) && isFinite(value))
        ) {
      ...
      set green(value) {
        this._green = this.constructor.normalizeRange(
          0,
          255,
          value,
          this._green
      ...
      set white(value) {
        this._white = this.constructor.normalizeRange(
          0,
          255,
          value,
          this.white
      ...
      set status(objLampState) {
        if (objLampState.power == true || objLampState.power == this.C_ON) {
          ...
          objLampState.power == false ||
          objLampState.power == this.C_OFF
        ) {
          this._power = this.C_OFF;
        }

        this._red = this.constructor.normalizeRange(
          0,
          255,
          objLampState.color.red,
          this._red
        );
        this._green = this.constructor.normalizeRange(
          0,
          255,
          objLampState.color.green,
          this._green
        );
        this._blue = this.constructor.normalizeRange(
          0,
          255,
          objLampState.color.blue,
          this._blue
        );
        this._white = this.constructor.normalizeRange(
          0,
    ...
    let light = new MagicHomeControl(host, {
      ack: {
        ...
        color: colorAck,
        pattern: patternAck,
        customPatternAck: customPatternAck,
      },
    });
    ...
    function setColor(r, g, b) {
      ...
      for (let color in arguments) {
        targetColors.push(
          LampState.normalizeRange(
            0,
            255,
            arguments[color],
            internalState.getColorAsArray()[color]
          )
        );
      }

      light.setColorAndWarmWhite(
        targetColors[0],
        targetColors[1],
        targetColors[2],
        internalState.white,
